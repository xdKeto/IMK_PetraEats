...
if (isset($_POST['emailInput']) && isset($_POST['passwordInput'])) {
    ...
    $_password = $_POST['passwordInput'];

    // Perform hard-coded check for email and password
    if ($_email == $email && $_password == $passowrd) {
        // Set session username and redirect to mahasiswa landing page
        $_SESSION['username'] = $_email;
        header('Location: mahasiswa_landing_page.php');
        exit();
    } else {
        // Display error message and redirect back to login page with error
        $errorMsg = "Email or password is incorrect. Please try again.";
        $_SESSION['error'] = $errorMsg;
        header('Location: mahasiswa_login.php?error=' . urlencode($errorMsg));
        exit();
    }
}
?>
        ...
        <div class="row custom-row">
          <div class="col-auto col-sm-auto col-md-6 col-lg-4 text-black">
            <div class="container-semua d-flex align-items-center h-custom-2 px-4 ms-xl-4 mt-5 pt-5 pt-xl-0 mt-xl-5">
              ...
              <form class="form-container" style="width: 30rem" accept-charset=" UTF-8" name="loginForm" action="mahasiswa_login.php" method="post">
                ...
                <div class="form-outline mb-4">
                  ...
                  <input required type="email" id="email" class="form-control form-control-lg" name="emailInput" />
                  <label class="email form-label" for="email">Email address</label>
                </div>

                <div class="form-outline mb-4">
                  <input required type="password" id="password" class="form-control form-control-lg" name="passwordInput" />
                  <label class="password form-label" for="password">Password</label>
                </div>

                <div class="signin-con pt-1 mb-4">
                  <button class="btn btn-lg btn-block" type="submit" name="login">Sign in</button>
                </div>

                <p class="forgot small mb-5 pb-lg-2"><a class="text-muted" href="https://sso.petra.ac.id/petrasso/changepass.php">Forgot password?</a></p>
              </form>
